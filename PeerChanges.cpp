...
void* serverThread(void * arg)
{
	...
	while(1)
	{
		...
		{
			bzero(buffer,sizeof(buffer));	
			fstream ff("/home/viviek/Desktop/OS_Assignment_2/Peer1/list_files.txt");
			string ss,temp="";
			while(getline(ff,ss))
			{
				if(temp=="")
		...
		else if(strstr(buffer,"lookForFile"))
		{
			string x(buffer);	
			cout<<x<<endl;
			string fname,word;
			...
			while(getline(ff,ss))
			{	
				if(ss.compare(fname)==0)
					fg="1";
			}	
			// cout<<temp<<endl;
			// cout<<fg<<endl;
			strcpy(buffer,fg.c_str());
...
void* uploadFile(void * arg)
{
	...
	string d="/home/viviek/Desktop/OS_Assignment_2/Peer1";
	string fn;
    ...
    while (1) {
     ...
     fstream wFile("/home/viviek/Desktop/OS_Assignment_2/Peer1/list_files.txt",ios_base::out | ios_base::app);
     wFile << fn;
     wFile.close();
...
int main(int argc, char *argv[])
{
	...
	arrg.s1 = argv[1];								//IP ADDRESS OF PEER
	arrg.s2 = argv[2];								//PORT_NO OF PEER
